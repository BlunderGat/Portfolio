...
std::string MYpolynom::toString() const {
    std::string result = "";
    for (int i = coefficients_array.getSize() - 1; i >= 0; --i) {
        if (i < coefficients_array.getSize() - 1 && coefficients_array[i] >= 0) {
            result += "+";
        }
        result += std::to_string(coefficients_array[i]) + "x^" + std::to_string(i);
    }
    return result.empty() ? "0" : result;
}

void MYpolynom::addMonomial(double coeff, unsigned degree) {
